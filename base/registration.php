...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (mysqli_num_rows($res) === 0) {
        ...
        if ($result) {
            ...
            error_log("response = $response[0] $response[1]\n " . mysqli_error($conn), 3, "d:\\OSPanel\\domains\\localhost\\logs\\error.log");

            echo json_encode($response);
        } else {
            error_log("Error inserting user: " . mysqli_error($conn), 3, "d:\\OSPanel\\domains\\localhost\\logs\\error.log");
